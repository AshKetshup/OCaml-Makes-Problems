...
module Lexer_regexp = struct
  ...
  and __ocaml_lex_tokenize_rec lexbuf __ocaml_lex_state =
    ...
    | 1 ->
      let s = Lexing.sub_lexeme_char lexbuf lexbuf.Lexing.lex_start_pos in
      ( CHAR s )
...
let regexp st =
  ...
  regexpr Lexer_regexp.tokenize linebuf
...
let saida inicio fim =
  match inicio with
  | -1 -> Printf.printf "NO"
  | _  -> Printf.printf "%d %d" inicio fim


let () : unit=
  print_endline "\n\nStart\n\n";
  let regex, adn = entrada () in
  print_endline "\n\nInput successful\n\n"

  
  let r = regexp a in
  print_endline "\n\nRegexp successfully read\n\n"
